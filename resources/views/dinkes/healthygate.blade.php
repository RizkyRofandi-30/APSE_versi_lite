    ...
    <div class="flex h-screen">
        ...
        <div class="w-64 shadow-lg text-white flex flex-col" style="background-color:#008A42;">
            ...
            <nav class="mt-6 space-y-1 flex-1">
                ...
                <a href="{{ route('dinkesMedlicense') }}" class="flex items-center px-4 py-3 hover:bg-white/20 transition text-white">
                    <i class="fas fa-chart-bar mr-3"></i> Medical License
                </a>
            
                <a href="{{ route('dinkesHealthygate') }}" class="flex items-center px-4 py-3 bg-white text-green-700 font-semibold">
                ...
                <div class="overflow-x-auto bg-white shadow rounded-lg border mb-12">
                    <table class="min-w-full text-sm text-center">
                        ...
                        <tbody>
                            @forelse ($perpanjangan as $p)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 border text-blue-600 font-semibold">
                                        <button data-id="{{ $p->id }}" data-jenis="{{ ucwords($p->jenis_izin) }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="py-4 border text-gray-500">
                        ...
                        <tbody>
                            @forelse ($permohonan as $d)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 border">{{ ucwords($d->jenis_usaha) }} </td>
                                    <td class="px-4 py-3 border">
                                        {{ \Carbon\Carbon::parse($d->created_at)->format('d M Y') }}
                                    </td>

                                    <td class="px-4 py-3 border text-blue-600 font-semibold">
                                        <button data-id="{{ $d->id }}" data-jenis="{{ ucwords($d->jenis_usaha) }}"
                                            onclick="openFormPerizinan(this.dataset.id, this.dataset.jenis)"
                                            class="bg-gray-600 text-white px-3 py-1 rounded hover:bg-gray-700">
                                            Berkas
                                        </button>
                ...
                <div class="relative w-full max-w-md rounded-xl shadow-2xl text-center p-5 bg-[#F5F5F5]">
                    ...
                    <form id="formSetujui" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="flex flex-col sm:flex-row gap-4 justify-center md:justify-end pt-6 border-t mt-6">
                            ...
                            <button 
                                onclick="openTolakSurat(selectedId)"
                                type="button" class="px-8 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition">
                                Tolak
                            </button>
...
<script>
    ...
    function openFormPerizinan(id, jenis) {
        ...
        // SET ACTION SETUJUI → /dinkes/healthygate/setujui/{id}
        const formApprove = document.getElementById("formSetujui");
        if (formApprove) {
            formApprove.action = "/dinkes/healthygate/setujui/" + selectedId;
        }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        formTolak.addEventListener("submit", function (e) {
            ...
            // FIX ROUTE YG BENAR
            this.action = "/dinkes/healthygate/tolak/" + selectedId;
        });
    });
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        formApprove.addEventListener('submit', function (e) {
            ...
            // set URL
            this.action = "/dinkes/healthygate/setujui/" + selectedId;
