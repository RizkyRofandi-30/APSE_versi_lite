...
<body class="bg-gray-100">
    ...
    <div class="flex h-screen">
        ...
        <!-- SIDEBAR -->
        <div class="w-64 shadow-lg text-white flex flex-col" style="background-color:#008A42;">

            <!-- Logo -->
            <div class=" p-4 border-b border-white/30 flex justify-center">
                <img src="{{ asset('images/logo-dinkes.png') }}" alt="Logo" class="w-40">
            </div>

            <nav class="mt-6 space-y-1 flex-1">

                <!-- Page aktif -->
                <a href="{{ route('dinkesMedlicense') }}" class="flex items-center px-4 py-3 bg-white text-green-700 font-semibold">
                    <i class="fas fa-chart-bar mr-3"></i> Medical License
                </a>

                <a href="{{ route('dinkesHealthygate') }}" class="flex items-center px-4 py-3 hover:bg-white/20 transition text-white">
        ...
        <div class="flex-1 flex flex-col bg-white overflow-y-auto">
            ...
            <div class="bg-white h-16 flex items-center justify-between p-8">
                <h1 class="text-4xl font-bold text-gray-800">Medical License</h1>
                <a href="#" class="font-medium">Admin DinKes</a>
            </div>
                    ...
                    <table class="min-w-full text-sm text-center">
                        <thead class="bg-gray-100 font-semibold text-gray-700">
                            <tr>
                                ...
                                <th class="px-4 py-3 border text-center">Surat Rekomendasi Dinkes</th>
                        ...
                        @forelse ($perpanjangan as $p)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 border">{{ ucwords($p->jenis_izin) }}</td>
                                ...
                                <td class="px-4 py-3 border text-center">
                                    <button
                                        data-id="{{ $p->id }}"
                                        data-jenis="{{ $p->jenis_izin }}"
                                        data-profesi="{{ $p->jenis_profesi }}"
                                        onclick="openUploadSurat(
                                            this.dataset.id,
                                            this.dataset.jenis,
                                ...
                                <td class="px-4 py-3 border">{{ $p->status ?? 'Sedang Diproses' }}</td>
                    ...
                    <table class="min-w-full text-sm text-center">
                        <thead class="bg-gray-100 font-semibold text-gray-700">
                            <tr>
                                ...
                                <th class="px-4 py-3 border">Tanggal Pengajuan</th>
                                <th class="px-4 py-3 border">Proses Berkas</th>
                                <th class="px-4 py-3 border">Surat Rekomendasi Dinkes</th>
                        ...
                        @forelse ($permohonan as $d)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 border">{{ ucwords($d->jenis_profesi) }}</td>
                                ...
                                <td class="px-4 py-3 border text-center">
                                    <button
                                        data-id="{{ $d->id }}"
                                        data-jenis="{{ $d->jenis_izin }}"
                                        data-profesi="{{ $d->jenis_profesi }}"
                                        onclick="openUploadSurat(
                                            this.dataset.id,
                                            this.dataset.jenis,
                                            this.dataset.profesi
                ...
                <div class="relative w-full max-w-sm rounded-xl shadow-2xl text-center p-5 bg-[#F5F5F5]">
                    ...
                    <form id="formSetujui" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="flex flex-col sm:flex-row gap-4 justify-center md:justify-end pt-6 border-t mt-6">
                            ...
                            {{-- Setujui --}}
                            <button type="submit" class="px-8 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition">
                                Ajukan Rekomendasi
                            </button>
                        </div>
                    </form>
                ...
                <div class="bg-white p-6 rounded-xl w-full max-w-sm shadow-2xl text-center">
                    ...
                    <div class="space-y-3 mb-4 text-left">
                        <label class="flex items-center cursor-pointer gap-3">
                            <input type="radio" name="status" value="dinkes-tidak-rekomen"
                            ...
                            <span class="text-sm">
                                Dinas Kesehatan tidak memberikan rekomendasi
                            </span>
                        </label>
...
<script>
    ...
    function openFormPerizinan(id, jenis) {
        ...
        // // SET ACTION untuk SETUJUI
        // const formApprove = document.getElementById("formSetujui");
        // if (formApprove) {
        //     formApprove.action = "/dinkes/medlicense/setujui/" + selectedID;
        // }

        // // SET ACTION untuk TOLAK
        // const formReject = document.getElementById("formTolak");
        // if (formReject) {
        //     formReject.action = "/dinkes/medlicense/tolak/" + selectedID;
    }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        formTolak.addEventListener("submit", function (e) {
            ...
            if (selectedStatus.value === "dinkes-tidak-rekomen") {
                inputKeterangan.value = "Dinas Kesehatan tidak memberikan rekomendasi";
            } else {
            ...
            // FIX ROUTE YG BENAR
            this.action = "/dinkes/medlicense/tolak/" + selectedId;
        });
    });

    document.addEventListener('DOMContentLoaded', function () {
        const inputFile = document.getElementById('surat_rekomendasi');
        const formApprove = document.getElementById('formSetujui');
        ...
        formApprove.addEventListener('submit', function (e) {
            ...
            // set URL
            this.action = "/dinkes/medlicense/setujui/" + selectedId;
